...
class WhisperTranscriber:
    ...
    def setup_ui(self):
        ...
        progress_container = tk.Frame(progress_frame, bg='#cccccc', relief=tk.SOLID, borderwidth=1)
        progress_container.pack(fill=tk.X, padx=50)
        
        self.progress_bar = tk.Canvas(progress_container, height=20, bg='#e0e0e0', 
    ...
    def update_progress(self, percentage, text=""):
        ...
        # Force GUI update to get correct dimensions
        ...
        if bar_width <= 1:
            # Use default width if not yet rendered
            bar_width = 500
            
        if percentage > 0:
            # Draw progress bar
            fill_width = int((bar_width - 4) * percentage / 100)
            self.progress_bar.create_rectangle(2, 2, fill_width + 2, 18, 
                                             fill='#4CAF50', outline='')
    ...
    def transcribe(self):
        ...
        try:
            ...
            with open(audio_file_to_transcribe, 'rb') as file:
                ...
                # Only send output parameter if not default
                data = {}
                ...
                def make_request():
                    ...
                    try:
                        response = session.post(url, files=files, data=data, timeout=3600)
                    finally:
                        session.close()
